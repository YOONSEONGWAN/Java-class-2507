 ...
 *  	입력, 출력 (Input, Output)
 *  
 *  	- 어떤 대상으로부터 데이터를 메모리로 읽어 들이는 걸 입력이라고 한다.
 *  	- 프로그래밍 언어의 관전에서 메모리라는 것은 변수 or 필드 or 객체로 생각하면 된다.
 *  	- 데이터는 2진수로 이루어져 있지만 2진수 8묶음인 1바이트 단위로 생각하며 ㄴ된다.
 *  	- 1byte 는 총256가지의 값을 표현할 수 있다.
